...
mfiles = [in_dir + '/SSWRS_V3.02_NOC_FVCOM_NWEuropeanShelf_01dy_19930101-1200_RE.nc']
...
if 1:
  for f in range(0, len(ofiles)):
    print(ofiles[f] in file_checked, (f/len(ofiles)) * 100, '%')
    ...
    with nc.Dataset(ofiles[f], 'r') as nc_fid:
      ...
      try:
        dep_adj = np.ma.array(nc_fid.variables['DEPH_ADJUSTED'][:])
        dep_adj_qc = np.ma.array(nc_fid.variables['DEPH_ADJUSTED_QC'][:])
      except:
        dep_adj = np.ma.zeros(dep_tmp.shape)
        dep_adj_qc = np.ma.zeros(dep_tmp.shape)
      try:
        temp_adj = np.ma.array(nc_fid.variables['TEMP_ADJUSTED'][:])
        temp_adj_qc = np.ma.array(nc_fid.variables['TEMP_ADJUSTED_QC'][:])
      except:
        temp_adj = np.ma.zeros(temp_tmp.shape)
        temp_adj_qc = np.ma.zeros(temp_tmp.shape)
      ...
      try:
        sal_tmp = np.ma.array(nc_fid.variables['PSAL'][:])
        sal_qc = np.ma.array(nc_fid.variables['PSAL_QC'][:])
      except:
        sal_tmp = temp_tmp * 0 - 99
        sal_qc = np.ma.zeros(temp_qc.shape, dtype='b')

      try:
        sal_adj = np.ma.array(nc_fid.variables['PSAL_ADJUSTED'][:])
        sal_adj_qc = np.ma.array(nc_fid.variables['PSAL_ADJUSTED_QC'][:])
      except:
