...
ua = np.ma.mean(ua, axis=0)
va = np.ma.mean(va, axis=0)
...
# Get mean distance of bottom touch offshore. do this first to use as 
# mask for front distance

# mask the places where there is a bigger top bottom difference
#tb_rho = np.ma.masked_where(tb_rho < -0.1, tb_rho)
a = tri.LinearTriInterpolator(trio, tb_rho)
dr_dx, dr_dy = a.gradient(x, y)

mag_dr = ((dr_dx ** 2) + (dr_dy ** 2)) ** 0.5


# Get mean distance of front offshore
...
#cs2 = ax2.tripcolor(mx, my, triangles, mag_dr, vmin=0, vmax=0.0001, zorder=99)
cs2 = ax2.tripcolor(mx, my, triangles, tb_freq, vmin=0, vmax=100, zorder=99)
ax2.tricontour(mx, my, triangles, tb_freq, [50], zorder=99)
ax2.tricontour(mx, my, triangles, mean_sal, [35], colors='red', zorder=100)

#ax2.tricontour(mx, my, triangles, mean_s * 1000, [0.01, 0.02], zorder=99)



fig1.colorbar(cs2, cax=cax1, orientation='horizontal')
#cax1.set_xlabel('drho/dx')
...
ax3.tricontour(mx, my, triangles, tb_rho, [-0.1, -0.07, -0.05, -0.03, 0], zorder=99)
ax3.tricontour(mx, my, triangles, mean_sal, [35], colors='red', zorder=99)
...
fig1.savefig('./Figures/trapping.png')
