...
year = np.array([d.year for d in date_list])
y_ind = year == 2019
mean_t = np.ma.mean(mag_t_grad[y_ind, :], axis=0)
mean_s = np.ma.mean(mag_s_grad[y_ind, :], axis=0)
...
data = np.load(in_dir + 'amm7_n_front.npz', allow_pickle=True)
...
year = np.array([d.year for d in date_list_7])
y_ind = year == 2019

mean_t7 = np.ma.mean(mag_t_grad7[y_ind, :, :], axis=0)
mean_s7 = np.ma.mean(mag_s_grad7[y_ind, :, :], axis=0)
...
year = np.array([d.year for d in date_list_15])
y_ind = year == 2019

mean_t15 = np.ma.mean(mag_t_grad15[y_ind, :, :], axis=0)
mean_s15 = np.ma.mean(mag_s_grad15[y_ind, :, :], axis=0)
...
cax2 = fig1.add_axes([0.88, 0.69, 0.01, 0.27])
cax3 = fig1.add_axes([0.405, 0.19, 0.01, 0.32])
cax4 = fig1.add_axes([0.88, 0.19, 0.01, 0.32])
...
fig1.colorbar(cs2, cax=cax2, extend='min')
...
cax3.set_ylabel('Temperature Gradient ($^{\circ}$C km$^{-1}$)')
...
cax4.set_ylabel('Salinity Gradient (PSU km$^{-1}$)')
...
fig1.savefig('./Figures/grad_ts_2019.png')
