...
fn_dy = sorted(glob.glob(mjas + 'Model_Output_V3.02/Daily/1998/SSWRS_V3.02*dy*RE.nc'))
fn_hr = sorted(glob.glob(mjas + 'Model_Output_V3.02/Hourly/1998/SSWRS_V3.02*hr*RE.nc'))
...
u_v_advection_x = np.zeros((FVCOM['ua'].shape[0] -1, FVCOM['ua'].shape[1]))
u_v_advection_y = np.zeros((FVCOM['ua'].shape[0] -1, FVCOM['ua'].shape[1]))
...
#  depthlev_mod = -H * siglev_mod # should be negative
  ...
  tau_wind_u, tau_wind_v = calc_wind_tau(forcing_dir 
        + 'SSW_Hindcast_metforcing_ERA5_1999.nc', use_date, t1)
...
cs1 = ax1.tripcolor(mx, my, triangles, momentum_full_x[t1-1], vmin=-1e-4, vmax=1e-4, zorder=99)
ax1.set_title('Momentum Balance')

fig1.colorbar(cs1, cax=cax1)
cax1.set_ylabel('Acceleration (ms$^{-2}$)')

cs2 = ax2.tripcolor(mx, my, triangles, du_dt[t1-1], vmin=-1e-4, vmax=1e-4, zorder=99)
ax2.set_title('Du/Dt')
...
axes[2, 0].plot(date_list, bot_stress_x[:, ind] + wind_stress_x[:, ind])
axes[2, 0].plot(date_list, bot_stress_y[:, ind] + wind_stress_y[:, ind])
...
fig1.savefig('./Figures/momentum_balance_greens.png')
fig2.savefig('./Figures/momentum_time_greens.png')

fig3.savefig('./Figures/my_balance_greens.png')
