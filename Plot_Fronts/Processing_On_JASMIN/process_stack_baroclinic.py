...
for i in range(len(fn)):
  ...
  for t1 in range(FVCOM['zeta'].shape[0]):
    ...
    baroclin_u[c, :] = np.sum(thermal_wind_u * -d_depth, axis=0) / H
    baroclin_v[c, :] = np.sum(thermal_wind_v * -d_depth, axis=0) / H
...
np.savez(out_dir + 'stack_baroclinic_vel_mn.npz', baroclin_u=u_mn, baroclin_v=v_mn, barotrop_u=ut_mn, barotrop_v=vt_mn, date_list=date_mn, lon=lon, lat=lat, tri=triangles)
