...
ua = np.ma.zeros((len(fn) * 10, lonc.shape[0])) - 999
va = np.ma.zeros((len(fn) * 10, lonc.shape[0])) - 999
...
ua_mn = np.ma.zeros((len(yri) * len(mnti), ua.shape[1]))
va_mn = np.ma.zeros((len(yri) * len(mnti), va.shape[1]))
date_mn = np.zeros((len(yri) * len(mnti)), dtype=object)
count = 0
for i in range(len(yri)):
  for j in range(len(mnti)):
    ind = (yr == yri[i]) & (mnt == mnti[j])
    ua_mn[count] = np.ma.mean(ua[ind, :], axis=0)
    va_mn[count] = np.ma.mean(va[ind, :], axis=0)
    date_mn[count] = dt.datetime(yri[i], mnti[j], 1)
    count += 1

if part1:
  np.savez(out_dir + 'stack_uv_mn1.npz', ua=ua_mn, va=va_mn, date_list=date_mn, lonc=lonc, latc=latc, tri=triangles)
else:
  np.savez(out_dir + 'stack_uv_mn2.npz', ua=ua_mn, va=va_mn, date_list=date_mn, lonc=lonc, latc=latc, tri=triangles)
