...
def steric_c(sal, tp, depth, lat, prac=False):
  # Calculate the steric height based velocity 
  ...
  steric = (sw.geo_strf_dyn_height(sa, ct, pres, 0) / g)[srf_st, :]

  return steric

def barotropic(trio, ssh, g, f, x, y, triangles, fvcom_files):
    if 1:
      a = tri.LinearTriInterpolator(trio, ssh)
      dn_dx, dn_dy = a.gradient(x, y)
    else:
      # Use Greens
      ssh_e = fvcom.grid.nodes2elems(ssh, triangles)
      dzeta = physics.green_gauss_gradient_method(ssh_e, fvcom_files)
      dn_dx = dzeta[:, 0]
      dn_dy = dzeta[:, 1]
...
vars = ('lon', 'lat', 'latc', 'lonc', 'nv', 'siglay', 'siglev', 'h', 'Itime', 'Itime2')
...
barosteric_u_mn = np.ma.zeros((len(years) * 12, lon.shape[0])) - 999
barosteric_v_mn = np.ma.zeros((len(years) * 12, lon.shape[0])) - 999
...
for i in range(len(fn)):
  ...
  for t1 in range(FVCOM['zeta'].shape[0]):
    ...
    if c == 0:
      ...
      barosteric_u = np.ma.zeros((32, lon.shape[0])) - 999
      barosteric_v = np.ma.zeros((32, lon.shape[0])) - 999
    ...
    if mn1 != date_list[c].month:
      ...
      barosteric_u_mn[mc, :] = np.ma.mean(barosteric_u[:dc, :], axis=0)
      barosteric_v_mn[mc, :] = np.ma.mean(barosteric_v[:dc, :], axis=0)
      barosteric_u = None
      barosteric_v = None
    ...
    parts = barotropic(trio, steric, g, f, x, y, triangles, fvcom_files)
    barosteric_u[dc, :] = parts[0]
    barosteric_v[dc, :] = parts[1]
    dc = dc + 1
    c = c + 1

date_list = date_list[0:c]
date_mn[mc] = dt.datetime(date_list[c - 1].year, date_list[c - 1].month, 1)
barosteric_u_mn[mc, :] = np.ma.mean(barosteric_u[:dc - 1, :], axis=0)
barosteric_v_mn[mc, :] = np.ma.mean(barosteric_v[:dc - 1, :], axis=0)

print(date_mn[-2:])


np.savez(out_dir + 'stack_barosteric_vel_mn.npz', barosteric_u=barosteric_u_mn, barosteric_v=barosteric_v_mn, date_list=date_mn, lon=lon, lat=lat, tri=triangles)
