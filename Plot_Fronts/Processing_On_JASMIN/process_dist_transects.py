...
sal_tran = np.ma.zeros((6, 2, len(fn) * 10, nbin)) - 999
temp_tran = np.ma.zeros((6, 2, len(fn) * 10, nbin)) - 999
ua_tran = np.ma.zeros((6, len(fn) * 10, nbin)) - 999
va_tran = np.ma.zeros((6, len(fn) * 10, nbin)) - 999
  ...
  for t1 in range(FVCOM['ua'].shape[0]):
    ...
    for t in range(sal_tran.shape[0]):
      ...
      ua_tran[t, c, :] = np.interp(
                              m_dist, m_dist[vsel], ua_tran[t, c, ivsel])
      va_tran[t, c, :] = np.interp(
                              m_dist, m_dist[vsel], va_tran[t, c, ivsel])

      for u in range(sal_tran.shape[1]):
        sel = sal_tran.mask[t, u, c, :] == 0
        isel = np.nonzero(sel)[0]
        sal_tran[t, u, c, :] = np.interp(
                              m_dist, m_dist[sel], sal_tran[t, u, c, isel])
        temp_tran[t, u, c, :] = np.interp(
                              m_dist, m_dist[sel], temp_tran[t, u, c, isel])
...
np.savez(in_dir + 'transect.npz', sal_tran=sal_tran, temp_tran=temp_tran, ua_tran=ua_tran, va_tran=va_tran, date_list=date_list, lon=lon, lat=lat, m_dist=m_dist)
