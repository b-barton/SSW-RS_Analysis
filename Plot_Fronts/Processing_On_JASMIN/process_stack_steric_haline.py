...
barosteric_u_b_mn = np.ma.zeros((len(years) * 12, lon.shape[0])) - 999
barosteric_v_b_mn = np.ma.zeros((len(years) * 12, lon.shape[0])) - 999
...
for i in range(len(fn)):
  ...
  for t1 in range(FVCOM['zeta'].shape[0]):
    ...
    if c == 0:
      ...
      barosteric_u_b = np.ma.zeros((32, lon.shape[0])) - 999
      barosteric_v_b = np.ma.zeros((32, lon.shape[0])) - 999
    ...
    if mn1 != date_list[c].month:
      ...
      barosteric_u_b_mn[mc, :] = np.ma.mean(barosteric_u_b[:dc, :], axis=0)
      barosteric_v_b_mn[mc, :] = np.ma.mean(barosteric_v_b[:dc, :], axis=0)
    ...
    _, steric_haline = steric_ab(
                FVCOM['salinity'][t1, :, :], 
                FVCOM['temp'][t1, :, :], 
                depth_mod, lat)

    barosteric_u_b[dc, :], barosteric_v_b[dc, :] = barotropic(trio, steric_haline, g, f, x, y)
    dc = dc + 1
    c = c + 1

date_list = date_list[0:c]
date_mn[mc] = dt.datetime(date_list[c - 1].year, date_list[c - 1].month, 1)
barosteric_u_b_mn[mc, :] = np.ma.mean(barosteric_u_b[:dc - 1, :], axis=0)
barosteric_v_b_mn[mc, :] = np.ma.mean(barosteric_v_b[:dc - 1, :], axis=0)

print(date_mn[-2:])


np.savez(out_dir + 'stack_barosteric_haline_vel_mn.npz', barosteric_u_b=barosteric_u_b_mn, barosteric_v_b=barosteric_v_b_mn, date_list=date_mn, lon=lon, lat=lat, tri=triangles)
